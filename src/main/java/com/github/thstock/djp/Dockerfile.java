...
public class Dockerfile {
  ...
  private Map<DockerfileLine,String> tokenLines1;
  private String from;
  private String cmd;
  ...
  public Dockerfile(File file, boolean strict) {
  ...
  Dockerfile(ImmutableList<String> allLines, boolean strict) {
    ...
    try {
      from = XStream.from(tokenLines)
          ...
          .last();
         System.out.println(XStream.from(tokenLines)
    	        .filter(l -> l.isToken(FROM))
    	        .map(DockerfileLine::getToken).mkString() + " = "+ from);
    ...
    try {
    	cmd = XStream.from(tokenLines)
          .filter(l -> l.isToken(CMD))
          .map(DockerfileLine::getValue).toList().toString();
          
    	
    	 System.out.println(XStream.from(tokenLines)
     	        .filter(l -> l.isToken(CMD))
     	        .map(DockerfileLine::getToken).toList().toString() + " = " +cmd);
    	
   
    } catch (NoSuchElementException e) {
      if (strict) {
        throw new IllegalStateException("Dockerfile must start with cmd");
      } else {
        cmd = "";
      }
    }
    ...
    try {
    	expose = XStream.from(tokenLines)
          .filter(l -> l.isToken(EXPOSE))
          .map(DockerfileLine::getValue).toList().toString();
          
    	
    	 System.out.println(XStream.from(tokenLines)
     	        .filter(l -> l.isToken(EXPOSE))
     	        .map(DockerfileLine::getToken).toList().toString() + " = " +expose);
    	
   
    } catch (NoSuchElementException e) {
      if (strict) {
        throw new IllegalStateException("Dockerfile must start with expose");
      } else {
    	  expose = "";
      }
    }
